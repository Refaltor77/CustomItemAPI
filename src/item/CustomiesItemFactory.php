...
final class CustomiesItemFactory {
	 ...
	 */
	public function registerItem(string $className, string $identifier, string $name, string $texture,int $maxStackSize, bool $allowOffHand, int $renderOffset): void {
		if($className !== Item::class) {
			Utils::testValidInstance($className, Item::class);
		}

		/** @var Item $item */
		$item = new $className(new ItemIdentifier(Cache::getInstance()->getNextAvailableItemID($identifier), 0), $name, $texture, $allowOffHand, $maxStackSize, $renderOffset);
    ...
    public function registerArmor(string $className, string $identifier, string $name,ArmorTypeInfo $info, string $texture, int $renderOffset): void {
        if($className !== Item::class) {
            Utils::testValidInstance($className, Item::class);
        }

        /** @var Item $item */
        $item = new $className(new ItemIdentifier(Cache::getInstance()->getNextAvailableItemID($identifier), 0), $name, $info, $texture, $renderOffset);
    ...
    public function registerFood(string $className, string $identifier, string $name, bool $isPotion, string $texture, int $maxStackSize, bool $canAlwaysEat, int $foodRestore, float $saturationRestore, int $renderOffset): void {
        if($className !== Item::class) {
            Utils::testValidInstance($className, Item::class);
        }

        /** @var Item $item */
        $item = new $className(new ItemIdentifier(Cache::getInstance()->getNextAvailableItemID($identifier), 0), $name, $info, $isPotion, $texture, $maxStackSize, $canAlwaysEat, $foodRestore, $saturationRestore, $renderOffset);
    ...
    public function registerTool(string $className, string $identifier, string $name, int $durability, float $efficiency, string $textureName, int $attack, int $renderOffset, ToolTier $tier): void {
        if($className !== Item::class) {
            Utils::testValidInstance($className, Item::class);
        }

        /** @var Item $item */
        $item = new $className(new ItemIdentifier(Cache::getInstance()->getNextAvailableItemID($identifier), 0), $name, $durability, $efficiency, $textureName, $attack, $renderOffset, $tier);
